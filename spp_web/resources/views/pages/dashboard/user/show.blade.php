        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col row">
                    ...
                    <div class="col">
                        <x-form.input.checkbox name="roles[]" :title="$role->name" :value="$role->id"
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <x-form.input.checkbox name="divisions[]" :title="$division->name" :value="$division->id"
                            id="{{ 'divisions_'.$division->id }}"
                            :checked="$user->divisions->contains('id', $division->id)" />
                    </div>
                    @endforeach
                </div>
            </div>
                ...
                <div class="row" id="village-checkboxes">
                    ...
                    <div class="col-md-3">
                        <x-form.input.checkbox name="villages[]" :title="$village->name" :value="$village->id"
                            id="{{ 'villages_'.$village->id }}"
                            :checked="$user->villages->contains('id', $village->id)" />
                    </div>
                    @endforeach
                </div>
                @error('villages')
...
<script>
    ...
    function toggleForms() {
        var divisionsFormVisible = areSpecificCheckboxesChecked(['roles_1', 'roles_2', 'roles_3']);
        var villagesFormVisible = areSpecificCheckboxesChecked(['roles_1', 'roles_2', 'roles_4']);
    ...
    userForm.addEventListener('submit', function(event) {
        ...
        if (!areSpecificCheckboxesChecked(['roles_1', 'roles_2', 'roles_3'])) {
            divisionsCheckboxes.forEach(function(checkbox) {
        ...
        if (!areSpecificCheckboxesChecked(['roles_1', 'roles_2', 'roles_4'])) {
            villagesCheckboxes.forEach(function(checkbox) {
                checkbox.disabled = true;
                checkbox.removeAttribute('name');
            });
