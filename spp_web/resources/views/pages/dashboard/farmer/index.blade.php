    ...
    <div class="card-body">
            ...
            <div class="card border-0">
                ...
                <div id="filterCard" class="collapse">
                    <div class="card-body p-2">
                        <x-form.input.text name="filter[name]" title="Name" :value="request()->input('filter.name')"
                            :in-line="true" />
                        <x-form.input.text name="filter[pic]" title="PIC" :value="request()->input('filter.pic')"
                            :in-line="true" />
                    </div>
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <div class="btn-group pagination">
                ...
                @endif
                @endif
                @for ($page = $startRange; $page <= $endRange; $page++) <a
                    class="btn btn-{{ $farmers->currentPage() === $page ? 'primary' : 'default' }}"
                            ...
                            @if ($endRange < $farmers->lastPage() - 1)
                                ...
                                @if ($farmers->currentPage() < $farmers->lastPage())
                                    <a class="btn btn-default"
                                        href="{{ $farmers->url($farmers->currentPage()+1) }}">Next</a>
                                    @endif
            </div>
            @endif
        </div>
